...
test("兩兩對戰頁設定與交接狀態沒有 axe 可偵測的 WCAG 2.2 A／AA 違規", async ({ page }, testInfo) => {
  await page.goto("/learn/versus");
  await expect(page.getByRole("heading", { name: "兩兩對戰（同一台裝置輪流）" })).toBeVisible();
  await expectNoAutomatedWcagViolations(page, testInfo, "versus-setup");

  await page.getByRole("button", { name: "開始對戰" }).click();
  await expect(page.getByRole("heading", { name: /輪到「玩家一」作答/ })).toBeVisible();
  await expectNoAutomatedWcagViolations(page, testInfo, "versus-handoff");
});

test("全班協力閘門頁沒有 axe 可偵測的 WCAG 2.2 A／AA 違規", async ({ page }, testInfo) => {
  await page.goto("/learn/class-battle");
  await expect(page.getByRole("heading", { name: "即時後端尚未連線" })).toBeVisible();
  await expectNoAutomatedWcagViolations(page, testInfo, "class-battle-gate");
});
...
test("診斷作答與答錯救援沒有 axe 可偵測的 WCAG 2.2 A／AA 違規", async ({ page }, testInfo) => {
  ...
  await expectNoAutomatedWcagViolations(page, testInfo, "diagnostic-warmup");
...
test("學生完整冒險的關鍵互動狀態沒有 axe 可偵測的 WCAG 2.2 A／AA 違規", async ({ page }, testInfo) => {
  ...
  await expectNoAutomatedWcagViolations(page, testInfo, "diagnostic-reveal");
